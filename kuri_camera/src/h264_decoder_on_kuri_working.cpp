...
void H264DecoderOnKuri::convertToMat() {
  while (is_alive) {
    //std::cout << "H264DecoderOnKuri::convertToMat" << std::endl;
    std::unique_lock<std::mutex> picture_lock(picture_mutex);
    got_picture_cvar.wait(picture_lock);
    //std::cout << "H264DecoderOnKuri::convertToMat after got pic" << std::endl;
...
bool H264DecoderOnKuri::load(std::string hostname, std::string port) {
  ...
  mdx_set_resolution(madmux_stream, 900, 600);
...
void H264DecoderOnKuri::h264_frame_callback(uint8_t *inbuf, uint32_t bytes_read) {
  ...
  //std::cout << "H264DecoderOnKuri::h264_frame_callback" << std::endl;
  ...
  int size = 0;
   ...
   if((size != 0 || len < 0) && len) {
    ...
    pkt.data = &(buffer[0]);
    pkt.size = size;//bytes_read;

    std::unique_lock<std::mutex> picture_lock(picture_mutex);
    int decodeLen = avcodec_decode_video2(codec_context, picture, &got_picture, &pkt);
    std::cout << "picture->pict_type " << picture->pict_type << std::endl;
    picture_lock.unlock();
