...
struct H264DecoderOnKuriNode {
  ...
  H264DecoderOnKuri* decoder_object;

  H264DecoderOnKuriNode(ros::NodeHandle &nh, H264DecoderOnKuri* h264_decoder) : transport(ros::NodeHandle()) {
    have_image = false;
    ...
    publish_image_thread = std::thread(&H264DecoderOnKuriNode::publishImage, this);
    cv_display_thread = std::thread(&H264DecoderOnKuriNode::cvDisplay, this);
    get_images_thread = std::thread(&H264DecoderOnKuriNode::getImages, this);
    // sample_function_thread = std::thread(&H264DecoderOnKuriNode::sampleFunctionForUsingImages, this);

    decoder_object = h264_decoder;
  }

  ~H264DecoderOnKuriNode() {
...
int main(int argc, char **argv) {
  ...
  H264DecoderOnKuri* h264_decoder = new H264DecoderOnKuri();
  H264DecoderOnKuriNode* node = new H264DecoderOnKuriNode(nh, h264_decoder);
