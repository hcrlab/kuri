#include "kuri_joystick_teleop.h"
...
KuriJoystickTeleop::KuriJoystickTeleop() :
  ...
  base_cmd_pub = nh.advertise<geometry_msgs::Twist>("mobile_base/commands/velocity", 1000); //"cmd_vel" may also work
  joystick_sub = nh.subscribe("joy", 1000, &KuriJoystickTeleop::joystickCallback, this);
  nav_vel_sub = nh.subscribe("kuri_navigation_command_velocity", 1, &KuriJoystickTeleop::navVelCallback, this);

  head_controller_state_sub = nh.subscribe("/head_controller/state", 1000, &KuriJoystickTeleop::headControllerStateCallback, this);
  eye_controller_state_sub = nh.subscribe("/eyelids_controller/state", 1000, &KuriJoystickTeleop::eyeControllerStateCallback, this);
  ...
  controlLoop_thread = std::thread(&KuriJoystickTeleop::controlLoop, this);
}
...
void KuriJoystickTeleop::controlLoop() {
...
void KuriJoystickTeleop::joystickCallback(const sensor_msgs::Joy &msg) {
    /*
...
void KuriJoystickTeleop::navVelCallback(const geometry_msgs::Twist &msg) {
  geometry_msgs::Twist vel;
...
void KuriJoystickTeleop::headControllerStateCallback(const control_msgs::JointTrajectoryControllerState & msg) {
...
void KuriJoystickTeleop::eyeControllerStateCallback(const control_msgs::JointTrajectoryControllerState & msg) {
...
int main(int argc, char **argv) {
	...
	KuriJoystickTeleop KuriJoystickTeleopNode;
